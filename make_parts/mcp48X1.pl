...
print make_qfp_pckg(
    ...
    add_fiducials  => 0,    # Don't add fiducial marks for placement.
);
    ...
    pckgs => [
        {
            ...
            variant    => '-MSOP8', # Package variant label (could be '' if only one footprint).
...
%pins = (
    ...
    properties         => {
        # The name and I/O type for each pin of the device.
        1 => { name => 'VDD', type => IN },
        2 => { name => 'CS#', type => IN },
        3 => { name => 'SCK', type => IN },
        4 => { name => 'SDI', type => IN },
...
# Now unite the device and pin descriptions into a complete device.
